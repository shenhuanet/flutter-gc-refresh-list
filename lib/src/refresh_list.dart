...
class GCRefreshListUtil {
  ...
  static void initConfig(GCRefreshListConfig config) {
    _config = GCRefreshListConfig(
        ...
        refreshingColor:
            config.refreshingColor ?? _defaultConfig.refreshingColor,
        ...
        emptyImageHeight:
            config.emptyImageHeight ?? _defaultConfig.emptyImageHeight,
        emptyWidget: config.emptyWidget ?? _defaultConfig.emptyWidget,
        initLoadingWidget:
            config.initLoadingWidget ?? _defaultConfig.initLoadingWidget,
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      if (controller.state.value == RefreshListState.initializing) {
        _child = config?.initLoadingWidget ??
            GCRefreshListUtil.config.initLoadingWidget!;
      }
      //第一次加载时出错
        ...
        _child = GestureDetector(
          ...
          child: Center(
            child: Text(
              "加载数据失败，点击重试",
              style: (config?.textStyle ?? GCRefreshListUtil.config.textStyle!)
      ...
      return RefreshConfiguration(
        ...
        child: SmartRefresher(
          ...
          header: ClassicHeader(
            height:
                config?.headerHeight ?? GCRefreshListUtil.config.headerHeight!,
            textStyle: config?.textStyle ?? GCRefreshListUtil.config.textStyle!,
            releaseText: '释放刷新',
            releaseIcon: Icon(Icons.refresh,
                color: config?.releaseColor ??
                    GCRefreshListUtil.config.releaseColor,
                size: 24),
            refreshingText: '刷新中...',
            refreshingIcon: SizedBox(
                child: CircularProgressIndicator(
                    strokeWidth: 2,
                    color: config?.refreshingColor ??
                        GCRefreshListUtil.config.refreshingColor),
                height: 20.0,
                width: 20.0),
            completeText: '刷新完成',
            completeIcon: Icon(Icons.check_circle,
                color: config?.completeColor ??
                    GCRefreshListUtil.config.completeColor,
                size: 20),
            failedText: '刷新失败,请重试',
            failedIcon:
          ...
          footer: ClassicFooter(
            ...
            loadingIcon: SizedBox(
                ...
                width: 20.0),
            noDataText:
                config?.noDataText ?? GCRefreshListUtil.config.noDataText,
            noMoreIcon:
                config?.noDataWidget ?? GCRefreshListUtil.config.noDataWidget,
            ...
            failedIcon: Icon(Icons.error,
                color:
                    config?.failedColor ?? GCRefreshListUtil.config.failedColor,
                size: 20),
            idleText: '上拉加载更多',
